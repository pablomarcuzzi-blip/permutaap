...
setTimeout(() => {
    ...
    const stmt = db.prepare(
        "INSERT INTO cargos_propuestos (nombre, tipo_escuela_descripcion, propuesto_por, estado, fecha_resolucion) VALUES (?, null, 1, 'aprobado', CURRENT_TIMESTAMP)"
    );

    cargos.forEach(n => {
        stmt.run(n, function(err) {
            if (err) console.log('Error en "' + n + '": ' + err.message);
            else insertados++;
        });
    });

    stmt.finalize(() => {
        console.log('Insertados: ' + insertados + ' cargos');
        process.exit();
    });
}, 1000);
